...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: paymentData, error: paymentError } = await supabase.from('payments').select('id,amount,currency,status,document_version_id,provider,provider_reference,metadata').eq('provider', 'wompi').eq('provider_reference', reference).maybeSingle();
    ...
    if (status === 'APPROVED') {
      ...
      if (payment.document_version_id) {
        const { data: documentData, error: documentError } = await supabase.from('documents').select('id,meta').eq('id', String(payment.document_version_id)).maybeSingle();
