...
export async function POST(request: NextRequest) {
  try {
    ...
    if (String(payment.currency || 'COP') !== 'COP' || expectedCents !== amountInCents) {
      return NextResponse.json({ error: 'Transaction amount does not match the stored payment.' }, { status: 409 });
    }

    const transactionId = String(transaction?.id || '');
    const metadata = {
      ...(payment.metadata || {}),
      wompi_transaction_id: transactionId || undefined,
      last_event_status: status,
      last_event_timestamp: payload.timestamp,
    };
