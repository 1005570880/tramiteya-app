...
export async function GET(req:NextRequest,{params}:{params:{id:string}}){
  try{
    ...
    if(row?.guest_access_token_hash){
      if(!guestToken || row.guest_access_token_hash!==guestHash)return NextResponse.json({error:'Acceso no autorizado.',code:'ACCESS_TOKEN_REQUIRED'},{status:403});
    } else {
      // First public visit establishes a high-entropy bearer credential for this result.
      guestToken=createGuestAccessToken();
      guestHash=hashGuestAccessToken(guestToken);
      const {error:updateError}=await supabase.from('procedure_instances').update({guest_access_token_hash:guestHash}).eq('id',params.id);
    ...
    response.cookies.set('tramiteya_guest_access',guestToken,{httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'lax',path:'/',maxAge:60*60*24*30});
    return response;
