...
export async function GET(req: NextRequest) {
  try {
    ...
    const list = await factory.getInstanceRepo().list();
    return NextResponse.json({ data: list.filter((i: any) => i.userId === user.id) });
  } catch {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Guest checkout: la instancia no requiere una cuenta.
    // Si existe sesión autenticada, se conserva la asociación al usuario.
    const inst = await factory.getInstanceRepo().create(
    ...
    console.error('Unable to create instance:', error);
