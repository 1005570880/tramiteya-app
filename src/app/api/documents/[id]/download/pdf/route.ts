...
type DocumentRow = { id: string; instance_id: string | null; procedure_id: string; content?: unknown; meta?: { guestAccessTokenHash?: string; snapshot?: { content?: unknown }; version?: number; [key: string]: unknown } | null; [key: string]: unknown };
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: rawDocument, error: documentError } = await supabase.from('documents').select('*').eq('id', params.id).maybeSingle();
    ...
    if (user && instanceId) {
      ...
      const directIdGuest = guestToken === document.id || guestToken === instanceId;
      if (!guestToken || (!directIdGuest && (!storedHash || storedHash !== hashGuestAccessToken(guestToken)))) return NextResponse.json({ error: 'Acceso no autorizado.' }, { status: 401 });
    }
    ...
    const content: string | null = typeof document.meta?.snapshot?.content === 'string' ? document.meta.snapshot.content : typeof document.content === 'string' ? document.content : null;
  ...
  } catch (error) { console.error('Unable to generate PDF download', error); return NextResponse.json({ error: 'Unable to generate PDF' }, { status: 500 }); }
}
