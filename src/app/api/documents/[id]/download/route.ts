...
async function hasApprovedPayment(userId: string, procedureId: string, documentVersionId?: string | null) {
  ...
  if (documentVersionId) {
    query = query.or(`document_version_id.eq.${documentVersionId},document_version_id.is.null`);
  }
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const versionParam = Number(new URL(req.url).searchParams.get('version') || 0);
    const doc = versionParam ? docs.find((d: any) => Number(d.version) === versionParam) : docs[docs.length - 1] || instance.document;
    ...
    return new NextResponse(buffer as unknown as BodyInit, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="tramiteya-${instance.procedureSlug}-v${doc.version || 1}.docx"`,
