...
async function hasApprovedPayment(documentVersionId: string) {
  ...
  const { data, error } = await supabase.from('payments').select('id,status').eq('document_version_id', documentVersionId).eq('status', 'approved').limit(1).maybeSingle();
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return new NextResponse(buffer as unknown as BodyInit, { status: 200, headers: { 'Content-Type': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'Content-Disposition': `attachment; filename="tramiteya-${instance.procedureSlug}-v${doc.version || 1}.docx"`, 'Cache-Control': 'no-store' } });
