...
export async function POST(request: Request) {
  try {
    ...
    if (typeof vertical !== 'string' || typeof procedure !== 'string' || typeof documentType !== 'string' || !facts || typeof facts !== 'object') {
      return NextResponse.json({ error: 'Faltan datos jurídicos para el análisis.' }, { status: 422 });
    }
    const result = await runLegalAiEngine({ vertical, procedure, facts: facts as Record<string, unknown>, documentType, draftingInstructions: typeof draftingInstructions === 'string' ? draftingInstructions : undefined });
