...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[SIMIT AUDIT] PDF received', JSON.stringify({ fileName: file.name, size: file.size, bufferLength: buffer.length }));

    let text = '';
    try {
      const parsedPdf = await pdf(buffer);
      text = String(parsedPdf?.text || '').trim();
      console.log('[SIMIT AUDIT] pdf-parse extraction', JSON.stringify({ textLength: text.length, pages: parsedPdf?.numpages || 0 }));
    } catch (pdfError) {
      console.error('[SIMIT AUDIT] pdf-parse failed', pdfError);
    }

    // Fallback: pdf-parse is the primary text-layer extractor. Keep the raw buffer
    // intact and fail explicitly rather than passing an unresolved Promise or inventing text.
    if (!text) {
      console.error('[SIMIT AUDIT] RAW TEXT EMPTY', JSON.stringify({ fileName: file.name, bufferLength: buffer.length }));
      return NextResponse.json({ ok: false, code: 'SIMIT_PDF_NO_TEXT', message: 'El PDF no contiene texto extraíble. Descarga nuevamente el Estado de Cuenta desde SIMIT.' }, { status: 422 });
    }

    console.log('[SIMIT AUDIT] RAW TEXT PREVIEW', JSON.stringify(text.slice(0, 3000)));
    console.log('[SIMIT AUDIT] RAW TEXT TAIL', JSON.stringify(text.slice(-1500)));
    ...
    console.log('[SIMIT AUDIT] deterministic parse result', JSON.stringify({ count: deterministicRecords.length, numbers: deterministicRecords.map(r => r.number) }));
    ...
    return NextResponse.json({ ok: true, source: 'SIMIT_STATEMENT_UPLOAD', extraction: process.env.OPENAI_API_KEY ? 'deterministic+ai-enrichment' : 'deterministic', extractionData: { documentNumber: documentNumber || null, plate: finalGlobalPlate || null, recordCount: hydratedRecords.length }, documentType: 'CC', documentNumber, ownerName, plate: finalGlobalPlate, fileName: file.name, recordCount: hydratedRecords.length, totalDebt, records: hydratedRecords, confidence: ai.confidence || 100, evidence: ai.evidence || [], message: `Estado de Cuenta SIMIT identificado. ${hydratedRecords.length} comparendos y multas encontrados.` });
  } catch (error) {
