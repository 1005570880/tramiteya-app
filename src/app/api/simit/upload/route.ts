...
function normalizeRecord(record: ExtractedRecord): ExtractedRecord | undefined {
  ...
  return { ...record, number, kind: String(record.kind ?? '').toLowerCase().includes('multa') ? 'multa' : 'comparendo', value: typeof record.value === 'number' && Number.isFinite(record.value) ? record.value : undefined };
}

function mergeEnrichment(base: ExtractedRecord[], ai: ExtractedRecord[]) {
  const byNumber = new Map(ai.map(raw => { const r = normalizeRecord(raw); return [r?.number || '', r]; }).filter(([number]) => Boolean(number)) as [string, ExtractedRecord][]);
  return base.map(raw => {
    const record = normalizeRecord(raw) || raw;
    const extra = byNumber.get(record.number || '');
    if (!extra) return record;
    const merged = { ...record };
    for (const key of Object.keys(extra) as (keyof ExtractedRecord)[]) {
      if ((merged[key] === undefined || merged[key] === null || merged[key] === '') && extra[key] !== undefined && extra[key] !== null && extra[key] !== '') merged[key] = extra[key] as never;
    }
    return merged;
  });
}
...
function inferDocumentNumber(text: string, ai: AiAnalysis) {
  ...
  const match = text.match(/(?:c[eé]dula|documento|identificaci[oó]n|CC)\s*(?:n[roº°.]?\s*)?[:\-]?\s*(?:\n\s*)?(\d{6,12})/i);
  if (match?.[1]) return match[1];
  const heading = text.match(/estado\s+de\s+cuenta\s*\n?\s*(\d{6,12})\s*\n?\s*fecha\s+de\s+expedici[oó]n/i);
  return heading?.[1] || '';
}
function extractTotal(text: string) {
  const match = text.match(/(?:total\s+(?:a\s+)?pagar|total\s+deuda|total\s+pendiente)[^$0-9]{0,40}\$?\s*([0-9.,]{4,})/i);
  return match?.[1] ? Number(match[1].replace(/[^0-9]/g, '')) : undefined;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Regla central: el parser determinístico decide qué registros existen.
    // La IA solo puede enriquecer campos ya identificados por el parser.
    const deterministicRecords = parseOfficialSimitText(text);
    if (!deterministicRecords.length) {
      return NextResponse.json({ ok: false, code: 'SIMIT_NO_DETERMINISTIC_RECORDS', message: 'El Estado de Cuenta SIMIT fue leído, pero su estructura de registros no pudo confirmarse. TrámiteYa no inventará comparendos.' }, { status: 422 });
    }
    ...
    const ownerName = ai.ownerName || records.find(r => r.ownerName)?.ownerName;

    if (ai.documentNumber && documentNumber && ai.documentNumber !== documentNumber) {
      return NextResponse.json({ ok: false, code: 'SIMIT_DOCUMENT_MISMATCH', message: 'La identidad encontrada en el documento no es consistente.' }, { status: 422 });
    }
