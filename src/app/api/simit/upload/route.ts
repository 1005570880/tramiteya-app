...
function looksLikeSimit(text: string) { const n = text.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ' '); const signals = ['estado de cuenta', 'comparendos y multas', 'simit'].filter(x => n.includes(x)).length; if (signals >= 2) return true; const hasIdentifier = /(?:\d{15,20}|\d{4}-FAD-\d+|TC-\d{4}-\d+|\d{4}-\d+-SA)/i.test(text); const hasDate = /\b\d{2}[/-]\d{2}[/-]\d{4}\b/.test(text); const hasKeyword = /resoluci[oó]n|comparendo|secretar[ií]a|infracci[oó]n|fecha/i.test(n); return hasIdentifier && hasDate && hasKeyword; }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // IMPORTANT: read the complete ArrayBuffer and await pdf-parse before any state/result is produced.
    ...
    let deterministicRecords = parseOfficialSimitText(text);
    if (!deterministicRecords.length) return NextResponse.json({ ok: false, code: 'SIMIT_NO_DETERMINISTIC_RECORDS', message: 'El Estado de Cuenta SIMIT fue leído, pero no se encontraron registros con evidencia suficiente. TrámiteYa no inventará comparendos.' }, { status: 422 });
