...
async function aiExtract(text: string): Promise<ExtractedRecord[]> {
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: process.env.OPENAI_SIMIT_MODEL || 'gpt-4o-mini',
        temperature: 0,
        messages: [
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!documentNumber) {
      return NextResponse.json({ ok: false, message: 'Ingresa la cédula antes de subir el Estado de Cuenta.' }, { status: 400 });
    }
    if (file.size > MAX_FILE_BYTES) {
      return NextResponse.json({ ok: false, message: 'El archivo supera el límite de 10 MB.' }, { status: 413 });
    }
    if (!(file.type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf'))) {
      return NextResponse.json({ ok: false, message: 'Sube únicamente el Estado de Cuenta en PDF descargado desde SIMIT.' }, { status: 415 });
    }
    ...
    if (!records.length) {
      return NextResponse.json({ ok: false, code: 'SIMIT_DOCUMENT_NOT_STRUCTURED', message: 'El PDF fue recibido, pero no se encontraron comparendos o multas identificables. Usa el Estado de Cuenta oficial descargado directamente desde SIMIT.' }, { status: 422 });
    }

    console.log('[SIMIT AUDIT] statement_upload', JSON.stringify({
      documentType: 'CC', documentNumber, fileName: file.name, mimeType: file.type,
      size: file.size, records: records.length, aiUsed: Boolean(process.env.OPENAI_API_KEY), timestamp: new Date().toISOString(),
    }));

    return NextResponse.json({
      ok: true, source: 'SIMIT_STATEMENT_UPLOAD', extraction: process.env.OPENAI_API_KEY ? 'hybrid' : 'deterministic',
      documentType: 'CC', documentNumber, fileName: file.name, records,
      message: `Estado de Cuenta analizado. Se encontraron ${records.length} registro(s). Selecciona uno para continuar.`,
    });
  } catch (error) {
