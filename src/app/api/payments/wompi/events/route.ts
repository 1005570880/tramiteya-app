...
export async function POST(request: Request) {
  try {
    ...
    const valid = verifyEventSignature(event, secret, request.headers.get('x-event-checksum'));
    if (!valid) return NextResponse.json({ error: 'Firma de evento inválida.' }, { status: 401 });
    ...
    if (!payment) return NextResponse.json({ received: true, ignored: 'unknown_reference' });
