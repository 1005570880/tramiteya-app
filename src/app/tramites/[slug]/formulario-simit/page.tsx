...
export default function SimitAutofillForm({ params }: { params: { slug: string } }) {
  ...
  async function handleUpload(file?: File) {
    if (!file) return;
    setError(""); setUploading(true); setRecords([]); setSelected(null); setFileName(file.name);
    try {
      ...
      setDocumentNumber(data.documentNumber || ""); setRecords(data.records || []);
  ...
  async function complete(answers: FormAnswers) {
    const issues = validateProcedureAnswers(currentProcedure, answers);
    ...
    try {
      ...
      if (supabase) { const { data: { session } } = await supabase.auth.getSession(); if (session?.user) { const r = await fetch("/api/instances", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ procedureId: currentProcedure.id, procedureSlug: currentProcedure.slug, answers: enriched }) }); if (r.ok) instance = await r.json(); } }
      if (!instance) instance = procedureStorage.create(currentProcedure.id, currentProcedure.slug, enriched);
      const r = await fetch("/api/documents/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ procedureSlug: currentProcedure.slug, answers: enriched, instanceId: instance.id }) });
      ...
      localDraftStorage.save(`procedure:${currentProcedure.slug}`, { data: enriched, savedAt: new Date().toISOString() });
      window.location.href = `/tramites/${currentProcedure.slug}/resultado/${instance.id}`;
