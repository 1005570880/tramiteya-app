    ...
    try {
      ...
      if (supabase) {
        ...
        if (session?.user) {
          ...
          const response = await fetch(`/api/instances/${params.id}`, { headers });
          if (response.ok) {
            const data = await response.json();
            setInstance(data);
            const historyResponse = await fetch(`/api/instances/${params.id}/documents`, { headers });
            ...
            if (latest?.id) {
              const paymentResponse = await fetch(`/api/payments?procedureId=${encodeURIComponent(data.procedureId || data.procedureSlug || params.slug)}&documentVersionId=${encodeURIComponent(latest.id)}`, { headers });
