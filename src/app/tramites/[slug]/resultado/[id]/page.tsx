...
function DocumentBody({ content, paid }: { content: string; paid: boolean }) {
  const blocks = useMemo(() => {
    ...
    const foundationIndex = lines.findIndex((line) =>
      /^(?:#{1,6}\s*)?(?:\*\*\s*)?(?:III\.?\s*)?FUNDAMENTOS DE DERECHO/i.test(line.trim()) ||
      /FUNDAMENTOS DE DERECHO/i.test(line.trim())
    );
  ...
  return (
    <div className="mx-auto w-full max-w-[850px] bg-white border border-slate-300 shadow-xl px-8 py-10 md:px-12 md:py-12 text-[15px] text-slate-900 leading-[1.3]" style={{ fontFamily: '"Arial Narrow", Arial, sans-serif' }}>
      <div className="whitespace-pre-wrap break-words">
        {blocks.map(({ line, blurred }, index) => {
          ...
          const className = blurred
            ? "relative select-none pointer-events-none filter blur-[5px] transition-all duration-500"
            : "";

          if (!trimmed) return <div key={index} className={`h-3 ${className}`} aria-hidden="true" />;
          if (isHeading) {
            return <h3 key={index} className={`text-lg font-bold my-4 ${className}`}>{headingText}</h3>;
          }
          if (/^[-•]\s+/.test(trimmed)) {
            return <p key={index} className={`my-1 pl-4 ${className}`}>{cleanInlineMarkdown(trimmed)}</p>;
          }
          if (/^\d+[.)]\s+/.test(trimmed)) {
            return <p key={index} className={`my-2 ${className}`}>{cleanInlineMarkdown(trimmed)}</p>;
          }
...
export default function ResultPage({ params }: { params: { slug: string; id: string } }) {
  ...
  async function load() {
    try {
      ...
      setInstance(local);
      if (local?.document?.id) {
        const paymentResponse = await fetch(`/api/payments?procedureId=${encodeURIComponent(local.procedureId || local.procedureSlug || params.slug)}&documentVersionId=${encodeURIComponent(local.document.id)}`, { headers: guestHeaders, cache: "no-store" });
        if (paymentResponse.ok) setPaid(Boolean((await paymentResponse.json()).approved));
      }
    } finally {
      setLoading(false);
          ...
          {tab === "preview" ? (
            <div className="mt-6">
              <div className="relative">
                ...
                {!paid && latest?.content && (
                  <div className="absolute left-1/2 top-[62%] z-20 w-[min(92%,460px)] -translate-x-1/2 -translate-y-1/2">
                    <div className="rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-2xl backdrop-blur-md ring-1 ring-black/5">
                      ...
                      <TrustBadges />
                      <div className="mt-5"><WompiCheckout procedureId={instance.procedureId || instance.procedureSlug || params.slug} documentVersionId={String(latest?.id || "")} onPending={() => undefined} /></div>
                    </div>
                  </div>
                )}
              </div>
