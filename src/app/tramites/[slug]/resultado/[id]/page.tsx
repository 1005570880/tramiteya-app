...
export default function ResultPage({ params }: { params: { slug: string; id: string } }) {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900 font-sans"><Header /><section className="max-w-5xl mx-auto px-4 py-12"><div className="bg-white p-6 md:p-8 rounded-2xl shadow">
    ...
    {tab === "preview" ? <div className="mt-6">
      ...
      {!paid && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-5"><div className="flex items-start gap-3"><div className="text-xl">🔒</div><div className="flex-1"><h2 className="font-bold text-lg">Documento listo para descargar</h2><p className="text-sm text-slate-600 mt-1">Puedes revisar gratuitamente el contenido completo. El pago habilita únicamente la descarga del documento final en Word y PDF.</p><div className="mt-4"><WompiCheckout procedureId={instance.procedureId || instance.procedureSlug || params.slug} documentVersionId={String(latest?.id || "")} onPending={() => undefined} /></div></div></div></div>}
