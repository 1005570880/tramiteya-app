...
export default function ResultPage({ params }: { params: { slug: string; id: string } }) {
  ...
  async function load() {
    try {
      ...
      const response = await fetch(`/api/instances/${params.id}${tokenQuery}`, { headers, cache: "no-store" });
      if (response.ok) {
  ...
  const answers: any = latest?.snapshot?.answers || instance.answers || {};
  const organismo = String(answers.organismo || answers.organismoTransito || answers.entidad || "").trim();
  const edit = () => router.push(`/tramites/${params.slug}/formulario?instance=${encodeURIComponent(instance.id)}`);
  const goToPayment = () => document.getElementById("payment-checkout")?.scrollIntoView({ behavior: "smooth", block: "center" });
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900 font-sans"><Header /><section className="max-w-5xl mx-auto px-4 py-12"><div className="bg-white p-6 md:p-8 rounded-2xl shadow">
    ...
    {tab === "preview" ? <div className="mt-6"><DocumentBlurPreview documentText={latest?.content || ""} organismo={organismo} onUnlock={goToPayment} />{!paid && <div id="payment-checkout" className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-5"><div className="flex items-start gap-3"><div className="text-xl">🔒</div><div className="flex-1"><div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-bold text-lg">Documento listo para descargar</h2><p className="text-sm text-slate-600 mt-1">El pago habilita la descarga del documento final en Word y PDF.</p></div><div className="shrink-0"><span className="text-xs font-bold uppercase tracking-wider text-slate-500">Total</span><div className="text-2xl font-black text-slate-950">$49.900 <span className="text-xs font-bold text-slate-500">COP</span></div></div></div><TrustBadges /><div className="mt-5"><WompiCheckout procedureId={instance.procedureId || instance.procedureSlug || params.slug} documentVersionId={String(latest?.id || "")} onPending={() => undefined} /></div></div></div></div>}</div> : <div className="mt-6 space-y-3">{docs.map((doc: any, i: number) => <div key={doc.id || i} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border rounded-xl p-4"><div><div className="font-semibold">Versión {doc.version ?? doc.meta?.version ?? i + 1}</div><div className="text-xs text-slate-500">{doc.generatedAt ? new Date(doc.generatedAt).toLocaleString("es-CO") : doc.createdAt}</div></div><div className="flex gap-2"><button disabled={!paid} onClick={() => download("docx", Number(doc.version ?? doc.meta?.version ?? i + 1))} className={paid ? "px-3 py-2 rounded-lg border text-sm font-medium" : "px-3 py-2 rounded-lg bg-slate-100 text-slate-400 text-sm font-medium cursor-not-allowed"}>Word</button><button disabled={!paid} onClick={() => download("pdf", Number(doc.version ?? doc.meta?.version ?? i + 1))} className={paid ? "px-3 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium" : "px-3 py-2 rounded-lg bg-slate-100 text-slate-400 text-sm font-medium cursor-not-allowed"}>PDF</button></div></div>)}</div>}
