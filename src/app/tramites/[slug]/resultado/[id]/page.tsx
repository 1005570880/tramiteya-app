...
export default function ResultPage({ params }: { params: { slug: string; id: string } }) {
  ...
  async function checkGuestPayment(documentId: string, procedureId: string) {
    const guestAccessToken = typeof window !== 'undefined' ? window.localStorage.getItem(guestTokenKey(documentId)) : null;
    if (!guestAccessToken) return false;
    const response = await fetch(`/api/payments?procedureId=${encodeURIComponent(procedureId)}&documentVersionId=${encodeURIComponent(documentId)}&guestAccessToken=${encodeURIComponent(guestAccessToken)}`, { cache: 'no-store' });
  ...
  async function load() {
    try {
      ...
      if (localDocument?.id) setPaid(await checkGuestPayment(String(localDocument.id), String(local?.procedureId || local?.procedureSlug || params.slug)));
  ...
  const download = async (format: "docx" | "pdf", version?: number) => {
    ...
    if (!session?.access_token) {
      const guestAccessToken = latest?.id && typeof window !== 'undefined' ? window.localStorage.getItem(guestTokenKey(String(latest.id))) : null;
      if (!guestAccessToken || !latest?.id) { alert("No encontramos el acceso seguro de tu compra en este navegador."); return; }
      const response = await fetch('/api/documents/guest-download', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ documentId: String(latest.id), guestAccessToken, format }) });
