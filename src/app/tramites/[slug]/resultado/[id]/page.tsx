...
export default function ResultPage({ params }: { params: { slug: string; id: string } }) {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900 font-sans"><Header /><section className="max-w-5xl mx-auto px-4 py-12"><div className="bg-white p-6 md:p-8 rounded-2xl shadow">
    ...
    {tab === "preview" ? <div className="mt-6">
      <div className="bg-slate-50 p-5 rounded-xl whitespace-pre-wrap text-sm leading-6 max-h-[560px] overflow-auto border">{latest?.content || "El contenido del documento no está disponible todavía."}</div>
      {!paid && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-5"><div className="flex items-start gap-3"><div className="text-xl">🔒</div><div className="flex-1"><div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-bold text-lg">Documento listo para descargar</h2><p className="text-sm text-slate-600 mt-1">Revisa gratuitamente el contenido. El pago habilita la descarga del documento final en Word y PDF.</p></div><div className="shrink-0"><span className="text-xs font-bold uppercase tracking-wider text-slate-500">Total</span><div className="text-2xl font-black text-slate-950">$49.900 <span className="text-xs font-bold text-slate-500">COP</span></div></div></div><TrustBadges /><div className="mt-5"><WompiCheckout procedureId={instance.procedureId || instance.procedureSlug || params.slug} documentVersionId={String(latest?.id || "")} onPending={() => undefined} /></div></div></div></div>}
