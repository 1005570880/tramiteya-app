...
export default function SimitUploadFirst({ slug }: { slug: string }) {
  ...
  return <div className="mt-6 rounded-2xl border border-blue-100 bg-blue-50 p-6">
    ...
    <div className="mt-4 rounded-xl border border-blue-200 bg-white p-4 text-sm text-slate-700"><strong>¿Qué necesitas?</strong><p className="mt-1">Ten a la mano el Estado de Cuenta oficial descargado desde SIMIT. El sistema utilizará únicamente la información contenida en ese documento.</p></div>
    <label className={`mt-5 flex cursor-pointer items-center justify-center rounded-xl px-5 py-4 text-center font-semibold text-white ${loading ? "bg-slate-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"}`}><input type="file" accept="application/pdf,.pdf" className="hidden" disabled={loading} onChange={e => { const file = e.target.files?.[0]; if (file) void upload(file); e.currentTarget.value = ""; }} />{loading ? "Analizando Estado de Cuenta..." : "Subir Estado de Cuenta SIMIT (PDF)"}</label>
    <p className="mt-2 text-xs text-slate-500">PDF · máximo 10 MB</p>{error && <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}
