    ...
    <>
      <div className="mb-5 rounded-2xl border border-blue-200 bg-white p-4 shadow-sm md:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Ayuda opcional · puedes omitirla</p><h3 className="mt-1 text-base font-black text-slate-950 md:text-lg">¿Necesitas ayuda para descargar el Estado de Cuenta de SIMIT?</h3><p className="mt-1 text-sm text-slate-600">Esta guía es completamente opcional. Si ya tienes tu PDF, <b>ignórala y súbelo directamente</b> en el recuadro de abajo.</p></div>
      ...
      {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-label="Guía para descargar el Estado de Cuenta de SIMIT">
        <div className="flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 md:px-6"><div><p className="text-xs font-bold uppercase tracking-wide text-blue-600">Guía de descarga · opcional</p><h2 className="text-lg font-black text-slate-950">Cómo obtener tu Estado de Cuenta de SIMIT</h2></div><button type="button" onClick={() => setOpen(false)} className="rounded-full px-3 py-1 text-2xl leading-none text-slate-400 hover:bg-slate-100" aria-label="Cerrar">×</button></div>
