...
export async function generateDocument({ procedure, answers }: { procedure: Procedure; answers: FormAnswers }): Promise<DocumentItem> { const generatedAt = new Date().toISOString(); return { id: generateId('doc'), title: `${procedure.title} - Documento generado`, procedureId: procedure.id, content: documentContent(procedure, answers), createdAt: generatedAt, generatedAt, version: 1, status: 'ready' }; }
