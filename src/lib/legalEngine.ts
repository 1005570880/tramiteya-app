...
export function assessTrafficRecord(record: SelectedRecordData): LegalAssessment {
  ...
  if (age !== null && age >= 1) {
    ...
    reasoning.push(hasResolution || hasHearing ? "Por la antigüedad del hecho debe verificarse el trámite contravencional y el término aplicable." : "La antigüedad del hecho y la ausencia de evidencia de audiencia o decisión en el Estado de Cuenta ameritan verificar la caducidad.");
  ...
  if (record.fechaMandamientoPago) {
    ...
    if (mandAge !== null && mandAge >= 5) {
      ...
      reasoning.push("La fecha reportada del mandamiento tiene cinco o más años; debe verificarse la firmeza del acto y las actuaciones de ejecución posteriores.");
  ...
  if (/fotodeteccion|fotomulta|c35|d02|c[aá]mara/.test(code.toLowerCase())) {
  ...
  const primaryRoute = uniqueRoutes.includes("PERDIDA_EJECUTORIEDAD") ? "PERDIDA_EJECUTORIEDAD" : uniqueRoutes.includes("PRESCRIPCION") ? "PRESCRIPCION" : uniqueRoutes.includes("CADUCIDAD") ? "CADUCIDAD" : uniqueRoutes.includes("FOTODETECCION") ? "FOTODETECCION" : "NOTIFICACION";
  return {
    routes: uniqueRoutes,
    primaryRoute,
...
export function generateLegalDraft(record: SelectedRecordData): LegalDraft {
  ...
  const hechos = [
    ...
    record.fechaResolucion ? `3. En la información disponible se identifica la resolución/acto de fecha ${record.fechaResolucion}.` : `3. En la información disponible no se identifica fecha de resolución sancionatoria, por lo que este aspecto debe verificarse en el expediente.`,
  ...
  const fundamentos = `La actuación se somete a revisión para determinar la procedencia de ${primary}. La valoración se realiza con base en la información disponible y no sustituye la revisión del expediente administrativo. La autoridad deberá verificar, según corresponda, los términos de caducidad y prescripción del régimen de tránsito, las garantías de notificación y debido proceso y las reglas de ejecutoriedad de los actos administrativos. No se afirma una causal como hecho cierto cuando falta evidencia indispensable para establecerla.`;
  const requests = [
