...
function mainRelief(route: string | null, a: LegalAssessment, r: SelectedRecordData): string {
  ...
  if (route === 'PRESCRIPCION') return configured(a)
    ? `Solicito que se declare la prescripción de la sanción y/o de la acción de cobro${id} y, como consecuencia, se termine la obligación, se archive cualquier actuación de cobro y se ordene la eliminación, cancelación o actualización del registro de la multa en el SIMIT y demás sistemas de información en los que figure, dentro del ámbito de competencia de la entidad.`
    : `Solicito que se verifique si se configuró la prescripción de la sanción y/o de la acción de cobro${id} y que, si así se establece al revisar el expediente, se declare la prescripción, se termine la obligación y se ordene la eliminación, cancelación o actualización del registro correspondiente en el SIMIT y demás sistemas de información en los que figure, conforme a las competencias legales de la entidad.`;
  if (route === 'CADUCIDAD') return `Solicito que se establezca si operó la caducidad de la actuación contravencional${id} y, de ser así, se declare, se dejen sin efectos las actuaciones que jurídicamente correspondan y se ordene la eliminación, cancelación o actualización del registro asociado.`;
  if (route === 'PERDIDA_EJECUTORIEDAD') return `Solicito que se determine si se configuró la pérdida de fuerza ejecutoria${id} y, de ser así, se declare, se termine el cobro y se ordene la eliminación, cancelación o actualización del registro correspondiente.`;
  if (route === 'FOTODETECCION') return `Solicito que se revise la legalidad de la imputación${id} y, si no se acredita mi responsabilidad personal con las garantías exigibles, se deje sin efectos la sanción y se ordene la eliminación, cancelación o actualización del registro correspondiente.`;
  if (route === 'NOTIFICACION' || route === 'DEBIDO_PROCESO') return `Solicito que se revise la regularidad de la actuación${id} y, si se acredita una irregularidad sustancial que afecte mi derecho de defensa o impida mantener sus efectos, se adopte la consecuencia jurídica correspondiente y se ordene la eliminación, cancelación o actualización del registro.`;
  if (route === 'REVOCATORIA_DIRECTA') return `Solicito que se revise la procedencia de la revocatoria directa${id} y, si se configuran sus presupuestos legales, se deje sin efectos la actuación o sanción y se ordene la eliminación, cancelación o actualización del registro correspondiente.`;
  return `Solicito que se revise integralmente la actuación${id} y que, si se establece una causal legal que impida mantener vigente la obligación, se adopte la decisión correspondiente y se ordene la eliminación, cancelación o actualización del registro.`;
}
...
function facts(r: SelectedRecordData, t: any): string {
  ...
  if (r.fechaResolucion) out.push(`${out.length + 1}. Se registra una resolución o acto sancionatorio de fecha ${r.fechaResolucion}, cuya copia y constancia de ejecutoria solicito verificar en el expediente.`);
  else out.push(`${out.length + 1}. El Estado de Cuenta que aporté no me permite conocer el número, contenido, fecha de expedición o ejecutoria de la decisión mediante la cual se habría impuesto la sanción.`);
  if (r.fechaNotificacion) out.push(`${out.length + 1}. Se registra una fecha de notificación (${r.fechaNotificacion}); necesito conocer qué actuación fue notificada y contar con la constancia correspondiente.`);
  else out.push(`${out.length + 1}. El Estado de Cuenta que aporté no me permite establecer la fecha ni el medio mediante el cual se habría notificado el acto sancionatorio.`);
  ...
  else out.push(`${out.length + 1}. No tengo acreditada la existencia ni la fecha de un mandamiento de pago relacionado con esta obligación.`);
  if (r.fechaNotificacionMandamiento) out.push(`${out.length + 1}. Se registra como fecha de notificación del mandamiento de pago el ${r.fechaNotificacionMandamiento}; solicito que se aporte la constancia que permita verificarla.`);
  else out.push(`${out.length + 1}. Tampoco tengo acreditada la fecha en que se habría notificado un mandamiento de pago.`);
...
function legalGrounds(a: LegalAssessment): string {
  ...
  const parts = ['El artículo 23 de la Constitución Política garantiza mi derecho a obtener una respuesta de fondo, clara y congruente. El artículo 29 protege mi derecho al debido proceso dentro de la actuación administrativa.'];
  if (routes.includes('PRESCRIPCION')) parts.push('El artículo 159 de la Ley 769 de 2002 establece el régimen de prescripción de las sanciones de tránsito y contempla la notificación del mandamiento de pago como actuación con incidencia en el término. La fecha y la constancia de esa notificación son determinantes para establecer si la obligación puede seguir siendo exigida.');
  if (routes.includes('CADUCIDAD')) parts.push('El artículo 161 de la Ley 769 de 2002 regula la caducidad de la acción por contravención de tránsito. Su aplicación exige verificar cuándo ocurrió el hecho y cuándo se produjo la decisión sobre la imposición de la sanción.');
  ...
  if (routes.includes('NOTIFICACION')) parts.push('Las actuaciones administrativas que deban notificarse deben contar con las constancias que permitan verificar el acto comunicado, el destinatario, el medio utilizado y la fecha de notificación.');
...
function requests(r: SelectedRecordData, a: LegalAssessment): string {
  ...
  const req = [
    ...
    '2. Que se me informe cuál fue la decisión mediante la cual se impuso la sanción, si existe, indicando número, fecha, contenido y constancia de ejecutoria, y que se me entregue copia íntegra.',
  ...
  if (route === 'PRESCRIPCION') {
    req.push('4. Que se me informe si existe o existió proceso de cobro coactivo y, en caso afirmativo, se me entregue copia del mandamiento de pago, de su constancia de notificación y de las actuaciones posteriores, con sus respectivas fechas.');
    req.push(configured(a) ? '5. Que se declare la prescripción de la sanción y/o de la acción de cobro y, como consecuencia, se termine la obligación, se archive cualquier actuación de cobro y se ordene la eliminación, cancelación o actualización del registro de la multa en el SIMIT y demás sistemas de información en los que figure, dentro de las competencias de la entidad.' : '5. Que se determine, con base en el expediente, si se configuró la prescripción de la sanción y/o de la acción de cobro y, si se encuentra configurada, se declare, se termine la obligación y se ordene la eliminación, cancelación o actualización del registro de la multa en el SIMIT y demás sistemas de información en los que figure.');
    req.push('6. Si la entidad considera que la prescripción no se ha configurado, solicito que indique la fecha de inicio y vencimiento que considera aplicables, la actuación que habría interrumpido el término, la fecha de su notificación y el soporte documental de cada circunstancia.');
  } else if (route === 'CADUCIDAD') req.push('4. Que se establezca si operó la caducidad y, de ser así, se declare, se adopten las consecuencias jurídicas correspondientes y se ordene la eliminación, cancelación o actualización del registro asociado.');
  else if (route === 'PERDIDA_EJECUTORIEDAD') req.push('4. Que se establezca si se configuró la pérdida de fuerza ejecutoria y, de ser así, se declare, se termine cualquier actuación de cobro que carezca de fundamento vigente y se ordene la eliminación, cancelación o actualización del registro correspondiente.');
    ...
    req.push('5. Que, si no se acredita legalmente mi responsabilidad personal o se establece una irregularidad que afecte la validez de la sanción, se deje sin efectos la actuación en lo que jurídicamente corresponda y se ordene la eliminación, cancelación o actualización del registro.');
  } else {
    ...
    req.push('5. Que, si se acredita una causal que impida mantener vigente la sanción o el cobro, se adopte la consecuencia jurídica correspondiente y se ordene la eliminación, cancelación o actualización del registro en los sistemas de información respectivos.');
  }
...
export function buildTrafficDocument(slug: string, a: FormAnswers) {
  ...
  return [
    ...
    'Respetados señores:',
    '',
    `En ejercicio del derecho fundamental de petición, solicito que se revise la situación jurídica de la actuación${number ? ` No. ${number}` : ' que figura registrada a mi nombre'}, con fundamento en la información que aporté y en el expediente administrativo.`,
    '',
    'I. OBJETO',
    '',
    ...
    '',
    assessment.temporal?.initialExpiryDate ? `La fecha del hecho es ${assessment.temporal.initialDate} y el vencimiento inicial calculado del término de tres años es ${assessment.temporal.initialExpiryDate}. Este cálculo debe confrontarse con el expediente y, si existe un mandamiento de pago, con su fecha efectiva de notificación.` : 'La situación jurídica definitiva debe establecerse con base en las fechas y documentos que integran el expediente administrativo.',
    '',
    'V. PETICIONES',
    '',
