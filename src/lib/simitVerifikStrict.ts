...
export async function lookupSimitByDocumentStrict(documentType:string,documentNumber:string):Promise<SimitLookupResult>{
  ...
  for(const r of directRecords){
    if(r.documentNumber){ if(r.documentNumber===dn) accepted.push(r); continue; }
    ...
    try{
      ...
      const detail=unwrap(detailRaw); const returnedDoc=doc(detail);
      // CRÍTICO: jamás heredamos la cédula consultada. Si Verifik no devuelve la cédula en el detalle,
      // el registro NO se considera verificado y no se muestra al usuario.
      if(!returnedDoc || returnedDoc!==dn){
        console.error('[SIMIT AUDIT] rejected_identity',JSON.stringify({documentNumber:dn,number:r.number,returnedDocument:returnedDoc??null,reason:returnedDoc?'document_mismatch':'detail_without_document'}));
        continue;
      }
      ...
      accepted.push({...r,documentNumber:returnedDoc,ownerName:(first(detail?.infractorComparendo,detail?.nombreCompleto,r.ownerName) as string|undefined),plate:(first(detail?.placaVehiculo,detail?.placa,r.plate) as string|undefined)});
    }catch{}
  }
  ...
  const personName=(generalDoc===dn?(first(general?.nombreCompleto,general?.nombre,general?.nombres) as string|undefined):undefined) || comparendos[0]?.ownerName;
  ...
  console.log('[SIMIT AUDIT] strict-normalized',JSON.stringify({documentType:dt,documentNumber:dn,candidates:directRecords.length,accepted:comparendos.length,generalDocument:generalDoc??null}));
