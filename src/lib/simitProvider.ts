...
function normalizeProviderResult(provider: Exclude<SimitLookupResult['provider'], 'official-manual'>, documentType: string, documentNumber: string, raw: any): SimitLookupResult {
  ...
  const items = toArray(data?.multas ?? data?.comparendos ?? data?.infracciones ?? data?.results ?? data?.items);
  ...
  const comparendos: SimitComparendo[] = items.map((item: any) => {
    ...
    return {
      number: String(firstDefined(item?.numeroComparendo, item?.NúmeroComparendo, item?.comparendoId, item?.numero, item?.number, item?.comparendo) ?? '').trim() || undefined,
