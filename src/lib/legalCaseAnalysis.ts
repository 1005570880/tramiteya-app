...
function parseDate(value?:string|null):Date|null { if(!value)return null; const raw=String(value).trim(); const dmy=raw.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{4})$/); const iso=dmy?`${dmy[3]}-${dmy[2].padStart(2,"0")}-${dmy[1].padStart(2,"0")}`:raw; const date=new Date(`${iso.slice(0,10)}T00:00:00Z`); return Number.isNaN(date.getTime())?null:date; }
...
export function analyzeTemporalCase(record:TemporalRecordInput):CaseLegalAnalysis {
  const initialExpiryDate=addYears(record.fecha,3), caducityExpiryDate=addYears(record.fecha,1);
  const mandamientoNotificationDate=record.fechaNotificacionMandamiento||null, postMandamientoExpiryDate=addYears(mandamientoNotificationDate,3);
  const ejecutoriaDate=record.fechaEjecutoria||null, ejecutoriaExpiryDate=addYears(ejecutoriaDate,5);
  const facts:string[]=[],inferences:string[]=[],rules:string[]=[],evidenceQuestions:string[]=[],events:CaseEvent[]=[],scenarios:TemporalScenario[]=[];

  if(hasValue(record.fecha)){ facts.push(`El registro aportado identifica como fecha del hecho ${record.fecha}.`); events.push({id:"hecho",label:"Hecho/infracción",date:record.fecha,status:"ACREDITADO",source:"Estado de Cuenta SIMIT / dato aportado",legalEffect:"Punto de partida para los cómputos temporales especiales de caducidad y prescripción."}); }
  ...
  if(caducityExpiryDate){
    ...
    events.push({id:"vencimiento-caducidad",label:"Vencimiento de caducidad calculado",date:caducityExpiryDate,status:"INFERIDO",source:"Cálculo jurídico sobre fecha del hecho",legalEffect:"Punto crítico para verificar si dentro del año se decidió sobre la imposición de la sanción y se produjo la actuación que legalmente interrumpe la caducidad."});
    rules.push("El artículo 161 de la Ley 769 de 2002 establece que la acción por contravención de las normas de tránsito caduca al año contado desde la ocurrencia de los hechos; debe verificarse la decisión y audiencia efectiva dentro del término legal.");
  }
  ...
  if(hasValue(record.fechaResolucion)||affirmative(record.existeResolucion)){ if(!record.fechaResolucion)facts.push("El registro indica la existencia de una resolución o acto sancionatorio, aunque su fecha no está acreditada."); events.push({id:"resolucion",label:"Acto sancionatorio",date:record.fechaResolucion||null,status:record.fechaResolucion?"ACREDITADO":"NO_ACREDITADO",source:"Dato del registro",legalEffect:"Permite trasladar el análisis a la sanción impuesta, su firmeza, notificación y exigibilidad."}); }
  else evidenceQuestions.push("Resolución o acto sancionatorio, fecha de expedición y constancia de ejecutoria.");

  if(hasValue(record.fechaNotificacion)){ facts.push(`Se reporta una fecha de notificación general: ${record.fechaNotificacion}.`); events.push({id:"notificacion",label:"Notificación reportada",date:record.fechaNotificacion||null,status:"ACREDITADO",source:"Dato del registro",legalEffect:"Debe verificarse qué acto fue notificado y si la constancia satisface las exigencias legales."}); }
  ...
  if(hasValue(record.fechaMandamientoPago)){ facts.push(`Se reporta mandamiento de pago de fecha ${record.fechaMandamientoPago}.`); events.push({id:"mandamiento",label:"Mandamiento de pago",date:record.fechaMandamientoPago||null,status:"ACREDITADO",source:"Dato del registro",legalEffect:"La fecha de expedición no equivale a su notificación y, por sí sola, no acredita interrupción del término de prescripción."}); }
  else evidenceQuestions.push("Existencia, fecha de expedición y copia íntegra del mandamiento de pago, si existe.");

  if(mandamientoNotificationDate){ facts.push(`Se reporta notificación del mandamiento de pago el ${mandamientoNotificationDate}.`); events.push({id:"notificacion-mandamiento",label:"Notificación del mandamiento",date:mandamientoNotificationDate,status:"ACREDITADO",source:"Dato del caso",legalEffect:"Interrumpe el término de prescripción, sujeto a la validez y eficacia de la notificación."); if(postMandamientoExpiryDate){ inferences.push(`Desde la notificación del mandamiento (${mandamientoNotificationDate}), el nuevo vencimiento calculado del término de tres años sería ${postMandamientoExpiryDate}, sin perjuicio de las actuaciones posteriores que deban examinarse.`); events.push({id:"vencimiento-post-mandamiento",label:"Nuevo vencimiento de prescripción calculado",date:postMandamientoExpiryDate,status:"INFERIDO",source:"Cálculo jurídico sobre notificación acreditada",legalEffect:"Punto de referencia para examinar la prescripción posterior al mandamiento."}); }}
  ...
  if(hasValue(record.fechaEjecutoria)){
    ...
    events.push({id:"ejecutoria",label:"Ejecutoria",date:record.fechaEjecutoria,status:"ACREDITADO",source:"Dato del caso",legalEffect:"Permite determinar desde cuándo el acto puede ser ejecutado y valorar su fuerza ejecutoria."});
    ...
    if(ejecutoriaExpiryDate){
      ...
      events.push({id:"vencimiento-ejecutoriedad",label:"Vencimiento de cinco años para pérdida de ejecutoriedad",date:ejecutoriaExpiryDate,status:"INFERIDO",source:"Cálculo jurídico sobre fecha de ejecutoria",legalEffect:"Punto crítico para verificar si durante los cinco años la autoridad realizó actos que legalmente correspondían para ejecutar el acto."});
      const e=parseDate(ejecutoriaExpiryDate),today=new Date();
      if(e&&e.getTime()<=today.getTime()){
        if(!hasValue(record.fechaMandamientoPago)&&!hasValue(record.actuacionesCobro)){
          inferences.push(`Han transcurrido cinco años desde la ejecutoria reportada sin que se aporte evidencia de actos de ejecución; existe una hipótesis objetiva de pérdida de ejecutoriedad que debe plantearse como excepción y verificarse con el expediente.`);
          scenarios.push({id:"ejecutoriedad-sin-ejecucion",title:"Cinco años sin actos de ejecución acreditados",condition:`Si desde la ejecutoria (${record.fechaEjecutoria}) y hasta ${ejecutoriaExpiryDate} la autoridad no realizó los actos que legalmente le correspondían para ejecutar el acto`,conclusion:"es viable solicitar el reconocimiento de la pérdida de ejecutoriedad y oponerse a la ejecución, conforme a los artículos 91 y 92 del CPACA."});
        } else {
          scenarios.push({id:"ejecutoriedad-con-actuaciones",title:"Existen actuaciones de ejecución",condition:`Si el expediente acredita actos de ejecución dentro de los cinco años siguientes a ${record.fechaEjecutoria}`,conclusion:"la pérdida de ejecutoriedad del numeral 3 del artículo 91 no puede darse por configurada solo por el transcurso del tiempo; debe identificarse cada actuación y su efecto jurídico."});
        }
      } else scenarios.push({id:"ejecutoriedad-en-curso",title:"Cinco años aún no cumplidos",condition:`Si aún no han transcurrido cinco años desde ${record.fechaEjecutoria}`,conclusion:"no procede afirmar actualmente la causal temporal del numeral 3 del artículo 91; sí procede conservar la cuestión para revisión si aparecen otras causales de pérdida de ejecutoriedad."});
    }
  } else {
    evidenceQuestions.push("Fecha exacta de ejecutoria o firmeza del acto sancionatorio; es indispensable para evaluar la causal de cinco años del artículo 91 del CPACA.");
  }

  if(hasValue(record.actuacionesCobro)){ facts.push(`El caso aporta información sobre actuaciones posteriores de cobro: ${record.actuacionesCobro}.`); events.push({id:"cobro",label:"Actuaciones posteriores de cobro",date:null,status:"ACREDITADO",source:"Dato aportado",legalEffect:"Debe determinarse si cada actuación tiene incidencia jurídica sobre la exigibilidad o el cómputo aplicable."}); }
  ...
  return {initialDate:record.fecha||null,initialExpiryDate,yearsTerm:initialExpiryDate?3:null,caducityExpiryDate,caducityStatus,mandamientoDate:record.fechaMandamientoPago||null,mandamientoNotificationDate,postMandamientoExpiryDate,ejecutoriaDate,ejecutoriaExpiryDate,ejecutoriaStatus:ejecutoriaDate?(ejecutoriaExpiryDate?"HIPOTESIS_OBJETIVA":"INDETERMINADO"):"INDETERMINADO",events,scenarios:[...new Map(scenarios.map(s=>[s.id,s])).values()],certainty,executiveSummary,temporalConclusion,evidenceQuestions:[...new Set(evidenceQuestions)],facts:[...new Set(facts)],inferences:[...new Set(inferences)],rules:[...new Set(rules)]};
}
