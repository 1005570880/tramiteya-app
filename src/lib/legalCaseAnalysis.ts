...
export function analyzeTemporalCase(record: TemporalRecordInput): CaseLegalAnalysis {
  ...
  if (hasValue(record.fecha)) {
    ...
    events.push({ id: "hecho", label: "Hecho/infracción", date: record.fecha, status: "ACREDITADO", source: "Estado de Cuenta SIMIT / dato aportado", legalEffect: "Punto de partida para el cómputo especial de prescripción de la sanción de tránsito." });
  } else {
    evidenceQuestions.push("Fecha exacta del hecho o infracción.");
  }

  if (initialExpiryDate) {
    inferences.push(`Aplicando un término inicial de tres años a la fecha del hecho, el vencimiento inicial calculado es ${initialExpiryDate}.`);
    events.push({ id: "vencimiento-inicial", label: "Vencimiento inicial calculado", date: initialExpiryDate, status: "INFERIDO", source: "Cálculo jurídico sobre fecha acreditada", legalEffect: "Punto crítico para determinar si existió una actuación interruptiva eficaz antes del vencimiento." });
    rules.push("La regla especial del artículo 159 de la Ley 769 de 2002 establece un término de tres años para la prescripción de las sanciones de tránsito, contado desde la ocurrencia del hecho, y prevé la interrupción con la notificación del mandamiento de pago.");
  }

  if (hasValue(record.fechaResolucion) || affirmative(record.existeResolucion)) {
    facts.push(record.fechaResolucion ? `Se reporta una decisión sancionatoria con fecha ${record.fechaResolucion}.` : "El registro indica la existencia de una resolución o acto sancionatorio, aunque su fecha no está acreditada.");
  ...
  if (hasValue(record.fechaMandamientoPago)) {
    ...
    events.push({ id: "mandamiento", label: "Mandamiento de pago", date: record.fechaMandamientoPago || null, status: "ACREDITADO", source: "Dato del registro", legalEffect: "Actuación relevante para el régimen de prescripción de la acción de cobro." });
  } else {
  ...
  if (mandamientoNotificationDate) {
    ...
    events.push({ id: "notificacion-mandamiento", label: "Notificación del mandamiento", date: mandamientoNotificationDate, status: "ACREDITADO", source: "Dato del caso", legalEffect: "Interrumpe el término de prescripción y da lugar al nuevo cómputo previsto por el régimen de cobro coactivo." });
    if (postMandamientoExpiryDate) {
      ...
      events.push({ id: "vencimiento-post-mandamiento", label: "Nuevo vencimiento calculado", date: postMandamientoExpiryDate, status: "INFERIDO", source: "Cálculo jurídico sobre notificación acreditada", legalEffect: "Punto de referencia para examinar la prescripción posterior al mandamiento." });
    }
  } else {
  ...
  if (initialExpiryDate) {
    scenarios.push({ id: "sin-interrupcion", title: "Sin actuación interruptiva acreditada", condition: `Si no existe una notificación válida del mandamiento de pago anterior al ${initialExpiryDate}`, conclusion: `existe una hipótesis objetiva de prescripción porque el término inicial habría llegado a su vencimiento el ${initialExpiryDate} sin que en la información disponible aparezca acreditada una interrupción eficaz.` });
  ...
  if (initialExpiryDate && !mandamientoNotificationDate) {
    ...
    if (initial && notification && notification.getTime() <= initial.getTime()) {
      ...
      certainty = "HIPOTESIS_OBJETIVA";
      temporalConclusion = `La notificación del mandamiento aparece situada el ${mandamientoNotificationDate}, después del vencimiento inicial calculado (${initialExpiryDate}). Esto constituye una hipótesis jurídicamente relevante de prescripción previa a la actuación de cobro, que debe ser confrontada con la fecha y validez de la notificación y con el expediente completo.`;
    }
  }

  const executiveSummary = initialExpiryDate
    ? `El análisis temporal parte de un hecho fechado ${record.fecha}, al que se aplica el término inicial de tres años. El vencimiento calculado es ${initialExpiryDate}. La existencia o ausencia de una notificación eficaz del mandamiento de pago antes de ese punto cambia el resultado jurídico y, por eso, debe tratarse como un hecho probatorio independiente.`
