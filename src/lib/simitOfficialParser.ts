...
function isRecordIdentifier(value: string) { const v = value.trim(); return /^(?:\d{10,24}|\d{4,}-[A-Z0-9]+(?:-[A-Z0-9]+)*|[A-Z]{1,12}-\d{3,}(?:-[A-Z0-9]+)*)$/i.test(v); }
...
function parseRows(text: string): ParsedSimitRecord[] {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (!number) continue;
    const standaloneNumber = /^\d{1,4}[.)]$/.test(lines[i]);
    const start = standaloneNumber ? i + 1 : i;
    const parts: string[] = [];
    ...
    const date = extractDate(body); const code = extractInfractionCode(body); const value = extractMoney(body);
    if (!date || !code || value === undefined) continue;
    const dateIndex = body.indexOf(date); const codeIndex = body.toUpperCase().indexOf(code);
    ...
    records.push({ kind: /cobro\s+coactivo/i.test(body) ? 'multa' : 'comparendo', number, date, authority, infractionCode: code, status: extractStatus(body), value });
  }
...
export function parseOfficialSimitText(input: string): ParsedSimitRecord[] { const text = normalizeText(input); if (!text) return []; const json = parseJsonRecords(text); return json || parseRows(text); }

export function parseOfficialSimitStatement(input: string): ParsedSimitStatement {
  const text = normalizeText(input); const records = parseOfficialSimitText(text);
