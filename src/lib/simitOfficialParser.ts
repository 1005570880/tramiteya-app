...
function extractMoney(value: string) {
  ...
  const candidates = [...value.matchAll(/(?<![A-Za-z0-9_-])([0-9]{1,3}(?:[.,][0-9]{3})+|[0-9]{4,})(?![A-Za-z0-9_-])/g)]
    .map(m => m[1]).filter(n => !/^20\d{2}$/.test(n) && !/^\d{10}$/.test(n) && !/^\d{20}$/.test(n));
...
function extractDelimitedFields(raw: string, date: string, code?: string, fallbackStatus?: string) {
  const idx = raw.indexOf(date); const after = raw.slice(idx >= 0 ? idx + date.length : 0);
  const cells = after.split('|').map(clean).filter(Boolean); let authority: string | undefined; let status: string | undefined;
  for (const cell of cells) {
    const s = extractStatus(cell); if (s && !status) status = s;
    if (!authority && cell && !TIME_RE.test(cell) && !/^\$?\s*[0-9.,\s]+$/.test(cell) && !(code && new RegExp(`^${code}$`, 'i').test(cell))) authority = sanitizeAuthority(cell, code);
  }
  if (!authority && code) { const codeIndex = after.toUpperCase().indexOf(code.toUpperCase()); if (codeIndex > 0) authority = sanitizeAuthority(after.slice(0, codeIndex), code); }
...
function parseRecord(number: string, chunk: string): ParsedSimitRecord | undefined {
  ...
  const code = extractCode(body); const detectedStatus = extractStatus(body); const fields = extractDelimitedFields(chunk, date, code, detectedStatus);
  const moneySource = body.replace(DATE_RE, ' ').replace(TIME_RE, ' ').replace(IDENTIFIER_RE, ' ').replace(/\b[A-D]\d{2}\b/gi, ' ');
  return {
    kind: /cobro\s+coactivo|\bmulta\b/i.test(body) ? 'multa' : 'comparendo', number,
    date, time: extractTime(body), authority: fields.authority,
    municipality: fields.authority, infractionCode: code, status: fields.status,
    value: extractMoney(moneySource), plate: 'No especificada en PDF'
  };
}

function parseRows(text: string) {
  const normalized = normalizeText(text); const records: ParsedSimitRecord[] = [];
  const anchors = [...normalized.matchAll(/\b\d{2}[/-]\d{2}[/-]\d{4}\s+\d{2}:\d{2}:\d{2}\b/g)];
  for (let i = 0; i < anchors.length; i++) {
    const start = anchors[i].index ?? 0; const end = i + 1 < anchors.length ? (anchors[i + 1].index ?? normalized.length) : normalized.length;
    const prefix = normalized.slice(Math.max(0, start - 800), start); const ids = [...prefix.matchAll(IDENTIFIER_GLOBAL_RE)];
    const number = ids.at(-1)?.[0]?.replace(/\s+/g, ''); if (!number) continue;
    let chunk = normalized.slice(start, end); const total = chunk.search(/\bTotal\s+(?:a\s+)?pagar\b/i); if (total >= 0) chunk = chunk.slice(0, total);
    const record = parseRecord(number, chunk); if (record) records.push(record);
  }
...
function enforceMinimumStructure(text: string) {
  ...
  if (!identifiers.length || !dates.length) return [] as ParsedSimitRecord[];
  const records: ParsedSimitRecord[] = [];
  identifiers.forEach((idMatch, index) => {
    const id = idMatch[0].replace(/\s+/g, ''); const idPos = idMatch.index ?? 0;
    const nextId = identifiers[index + 1]?.index ?? normalized.length;
    const dateMatch = dates.find(d => (d.index ?? -1) > idPos && (d.index ?? normalized.length) < nextId)
      || dates.find(d => (d.index ?? -1) > idPos) || dates.find(d => (d.index ?? -1) < idPos);
    if (!dateMatch) return;
    const datePos = dateMatch.index ?? 0; const windowStart = Math.min(idPos, datePos); let chunk = normalized.slice(windowStart, Math.min(normalized.length, Math.max(idPos, datePos) + 2200));
  ...
  if (!records.length && identifiers[0] && dates[0]) {
    ...
    const total = chunk.search(/\bTotal\s+(?:a\s+)?pagar\b/i); if (total >= 0) chunk = chunk.slice(0, total);
    const record = parseRecord(id, chunk); if (record) records.push(record);
    if (!records.length) records.push({ kind: 'comparendo', number: id, date: dates[0][0], time: extractTime(chunk), status: extractStatus(chunk) || 'Pendiente', plate: 'No especificada en PDF', infractionCode: extractCode(chunk), authority: sanitizeAuthority(chunk, extractCode(chunk)) });
  }
  return dedupe(records);
}
...
function dedupe(records: ParsedSimitRecord[]) {
  ...
  for (const r of records) { const key = `${r.number || ''}|${r.date || ''}`; const old = map.get(key); map.set(key, old ? { ...old, ...r, authority: r.authority || old.authority, value: r.value ?? old.value } : r); }
  return [...map.values()];
}

export function parseOfficialSimitText(input: string): ParsedSimitRecord[] {
  const text = normalizeText(input); if (!text) return [];
