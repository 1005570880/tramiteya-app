...
const SPLIT_ID_RE = /(?<!\d)((?:\d[\s|]*){20})(?!\d)/g;
...
function buildRecord(number: string, body: string): ParsedSimitRecord | undefined {
  ...
  if (!date || !code || extractMoney(withoutNumber) === undefined) return undefined;
  return { kind: /cobro\s+coactivo/i.test(body) ? 'multa' : 'comparendo', number, date, time: extractTime(body), municipality, authority: authorityFromMunicipality(municipality, body), plate: extractSimitPlate(body), infractionCode: code, status, value: extractMoney(withoutNumber) };
}
...
function findRecordIdentifiers(text: string): Array<{ number: string; index: number }> {
  ...
  const add = (number: string, index: number) => { if (!/^\d{20}$/.test(number)) return; const key = `${number}|${index}`; if (seen.has(key)) return; seen.add(key); found.push({ number, index }); };
  for (const match of text.matchAll(CONTIGUOUS_ID_RE)) add(match[0], match.index ?? 0);
  for (const match of text.matchAll(SPECIAL_ID_RE)) add(match[0].replace(/\s+/g, ''), match.index ?? 0);
  for (const match of text.matchAll(SPLIT_ID_RE)) {
    const number = compactDigits(match[1]); const index = (match.index ?? 0) + (match[0].length - match[1].length);
    if (!/^\d{20}$/.test(number)) continue;
    // A split identifier is accepted only when its own nearby record contains
    // the three evidentiary fields: date, infringement code and monetary value.
    const window = text.slice(index, Math.min(text.length, index + 320));
    if (!extractDate(window) || !extractCode(window) || extractMoney(window) === undefined) continue;
    add(number, index);
  }
  return found.sort((a, b) => a.index - b.index);
}

function parseAnchoredRecords(text: string): ParsedSimitRecord[] {
  const ids = findRecordIdentifiers(text); const records: ParsedSimitRecord[] = [];
  for (let i = 0; i < ids.length; i++) {
    const current = ids[i]; const end = ids[i + 1]?.index ?? text.length;
    let chunk = text.slice(current.index, end); const totalIndex = chunk.search(/\bTotal\s+(?:a\s+)?pagar\b/i); if (totalIndex >= 0) chunk = chunk.slice(0, totalIndex);
...
function dedupe(records: ParsedSimitRecord[]): ParsedSimitRecord[] {
  ...
  for (const record of records) { if (!record.number) continue; const previous = map.get(record.number); if (!previous) map.set(record.number, record); else map.set(record.number, { ...previous, ...record, date: record.date || previous.date, time: record.time || previous.time, authority: record.authority || previous.authority, municipality: record.municipality || previous.municipality, plate: record.plate || previous.plate, value: record.value ?? previous.value, infractionCode: record.infractionCode || previous.infractionCode, status: record.status || previous.status }); }
  return [...map.values()];
}

export function parseOfficialSimitText(input: string): ParsedSimitRecord[] {
  const text = normalizeWhitespace(input); if (!text) return [];
  return dedupe(parseAnchoredRecords(text));
}
